...
public class RunWhisper : MonoBehaviour
{
    ...
    public async void Start()
    {
        ...
        {
            Debug.Log("Detected microphone: " + (string.IsNullOrEmpty(device) ? "[Unnamed Device]" : device));
        }
        ...
        micDevice = Microphone.devices.Length > 0 ? Microphone.devices[1] : null;
        ...
        micClip = Microphone.Start(micDevice, false, 30, 16000);  // 30s max, 16kHz sample rate
        while (!(Microphone.GetPosition(micDevice) > 0)) { }  // Wait for mic to start

        // Set audioClip to use microphone input
        audioClip = micClip;
